...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: apiResponse } = await api.get("/producer/admin/coupons", {
      params: { eventId: parsed.data.eventId },
    });
    const validation = adminCouponsResponseSchema.safeParse(apiResponse.data);
    ...
    return NextResponse.json(
      { success: false, error: "Falha ao listar cupons" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data } = await api.post(
      "/producer/admin/create-coupon",
      parsed.data
    );
    return NextResponse.json({ success: true, data: data.data });
  } catch {
    return NextResponse.json(
      { success: false, error: "Falha ao criar cupom" },
      { status: 500 }
    );
  }
}
